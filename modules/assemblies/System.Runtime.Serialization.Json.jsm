 ...
 * the Initial Developer. All Rights Reserved.
 *
 * Contributor(s): Neil Stansbury <neil.stansbury@redbacksystems.com>
 *
 * Alternatively, the contents of this file may be used under the terms of
 * either the GNU General Public License Version 2 or later (the "GPL"), or
 * the GNU Lesser General Public License Version 2.1 or later (the "LGPL"),
...
var EXPORTED_SYMBOLS = ["DataContractJsonSerializer"];

Components.utils.import( "resource://xulu/assemblies/System.jsm" );
System.Import( "System.Runtime.Serialization" );


function DataContractJsonSerializer( type )	{

}
DataContractJsonSerializer.prototype = {
	ReadObject : function( stream )	{
		var buf = [];
		stream.Read( buf, 0, stream.Length );
		return JSON.fromString( buf[0].join("") ) ;
	},
	WriteObject : function( stream, graph )	{
		try {
			jsString = JSON.toString( graph );
			stream.Write( jsString, 0, jsString.length );
		}
		catch( ex )	{
